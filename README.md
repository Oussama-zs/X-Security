...
4. **AI Supervisor** : Corrélation des failles et élimination des **Faux Positifs** via un LLM. L'IA juge la criticité technique et propose des remédiations.
5. **Reporting** : Génération automatique d'un rapport PDF professionnel listant uniquement les vulnérabilités prouvées avec preuves de concept (PoC).

## ⚙️ Pré-requis et Installation

### Pré-requis
*   **Python 3.8+**
*   **Nuclei** : Doit être installé et accessible dans la variable d'environnement `PATH`.
*   **OWASP ZAP** : Le démon ZAP doit être lancé en arrière-plan sur le port local `8081`.
...
La syntaxe générale de X-SECURITY requiert les drapeaux de mode (`-H`, `-S`, ou `-D`), vos identifiants d'API, et le **chemin vers le dossier source en argument positionnel à la fin**.

### Mode Hybride (SAST + DAST simultanés) - Recommandé
C'est le mode le plus puissant. Il lance l'analyse statique sur le code ET les attaques dynamiques sur le serveur, puis l'IA consolide le tout.

```bash
xsecurity -H --target-url "http://127.0.0.1:8080" --dast-engines "wapiti,nuclei,zap" --api-url "https://api.anthropic.com/v1/messages" --model "claude-3-5-sonnet-20240620" --api-key "VOTRE_CLE_API" ./dummy_vulnerable_app
```
...
```bash
xsecurity -S --api-url "https://api.anthropic.com/v1/messages" --model "claude-3-5-sonnet-20240620" --api-key "VOTRE_CLE_API" ./mon_code_source
```

### Mode DAST Uniquement (Analyse Dynamique)
...
| Option | Description |
| :--- | :--- |
| `-H`, `--hybrid` | Active le scan Hybride (SAST + DAST). |
| `-S`, `--sast` | Active le scan Statique uniquement (Code Source). |
| `-D`, `--dast` | Active le scan Dynamique uniquement (URL). |
...
| `[target_dir]` | **(Argument positionnel)** Le chemin vers le dossier du code source à analyser (ex: `./mon_dossier`). |
